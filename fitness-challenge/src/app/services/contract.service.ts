...
export class ContractService {
  ...
  constructor(
    ...
    if (typeof window.ethereum !== 'undefined') {
      ...
      this.provider.on('network', (newNetwork: any, oldNetwork: any) => {
        ...
        } else if (newNetwork.name !== 'rinkeby') {
          alert('please use rinkeby network');
        }
      });
      this.web3provider.enable().then((address: string) => {
  ...
  private connectContract(provider: any) {
    this.challengeManager = new ethers.Contract(
      environment.challengeManagerAddress,
      environment.challengeManagerAbi,
      provider
    );
    this.challengeManager = this.challengeManager.connect(provider.getSigner());

    this.challenger = new ethers.Contract(
      environment.challengerAddress,
      environment.challengerAbi,
      provider
    );
  ...
  public async createChallenge(
    ...
    try {
      ...
      this.store.dispatch(
        setTransactionHash({ hash: receipt.transactionHash })
      );
    } catch (error: any) {
      this.store.dispatch(
        setTransactionError({
          error: `${error.reason}: ${error.code}`,
  ...
  public async submitChallenge(
    ...
    let challengeUnlocked = await this.challenger.hasUnlockedChallenge(
      id,
      this.userAddress
    );
    let args = {};
    if (!challengeUnlocked) {
      let gas = await this.challenger.estimateGas.submitData(
        id,
        [+data],
        [time],
        { value: fee }
