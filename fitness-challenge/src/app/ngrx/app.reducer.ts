...
export const appReducer = createReducer(
  ...
  on(setChallenges, (state, { challenges }) => ({
    ...state,
    challenges: challenges.map(toChallenge),
  })),
  on(setAddress, (state, { address }) => ({ ...state, address: address })),
  on(setConcept2Name, (state, { name }) => ({
    ...state,
    concept2: { ...state.concept2, name: name },
  })),
  on(setConcept2Data, (state, { data }) => ({
    ...state,
    concept2: {
      ...state.concept2,
      data: data.map((workout) => ({
        ...workout,
        date: workout.date_utc ? workout.date_utc : workout.date,
      })),
      loading: false,
    },
  })),
  on(setTrainingData, (state, { data }) => ({ ...state, trainingData: data })),
  on(setConcept2DataLoading, (state, { isLoading }) => ({
    ...state,
    concept2: { ...state.concept2, loading: isLoading },
  })),
  on(setDisplayedChallenge, (state, { id }) => ({
    ...state,
    displayedChallenge: id,
  })),
  on(setTransactionLoading, (state, { isLoading }) => ({
    ...state,
    transaction: { loading: isLoading, hash: '', error: '' },
  })),
  on(setTransactionError, (state, { error, hash }) => ({
    ...state,
    transaction: { loading: false, error: error, hash: hash },
  })),
  on(setTransactionHash, (state, { hash }) => ({
    ...state,
    transaction: { loading: false, error: '', hash: hash },
  }))
);
...
export const selectDisplayedChallenge = createSelector<any, any, any>(
  ...
  (state: AppState) =>
    state.challenges.find(
      (challenge) => challenge.id === state.displayedChallenge
    )
);
...
export const selectEndedChallenges = createSelector<any, any, any>(
  ...
  (state: AppState) =>
    state.challenges.filter(
      (challenge: Challenge) => challenge.end.valueOf() < new Date().valueOf()
    )
);
...
export const selectTrainingData = createSelector<any, any, any>(
  (reducer: any) => reducer.data,
  (state: AppState) =>
    state.concept2.data.map((result) => ({
      id: result.id,
      date: result.date,
      brand: 'concept2',
      type: result.type,
...
export const selectTrainingsForDisplayedChallenge = createSelector<
  any,
  any,
  any
>(
